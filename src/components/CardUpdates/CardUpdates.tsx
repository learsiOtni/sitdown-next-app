...
export default function CardUpdates({ data, className, view }: Readonly<CardUpdatesProps>) {
  ...
  return (
    <>
      { filteredUpdates.length > 0 ? (
        filteredUpdates.map((update) => <CardUpdate key={update.id} data={update} view={view} className={className}/>)
      ) : (
